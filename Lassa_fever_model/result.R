i<-1
file<-sprintf("best_lassa%3.3d.csv",i)
while(file.exists(file)==F & i<401)
file<-sprintf("best_lassa%3.3d.csv",i<-i+1)
if(i<401)
{
require('pomp.orig')
source('lassamodel.R')
...
colnames(y)[1]<-'town'
#print(y$loglik)
z<-read.csv('best_lassa0.csv',row.names=1)
tw<-row.names(z)
...
write.csv(z,'current_lassa.csv')
print(z[sort(z$loglik,dec=T,index=T)$ix,])
x<-read.csv('best_lassa0.csv',row.names=1)
y<-read.csv('current_lassa.csv',row.names=1)
